...
prototype = {
  ...
  assignBinding: function assignBinding (target, targetKeypath, viewKeypath) {
    if (typeof this.bindings[target.id] === 'undefined') {
      ...
      appSubsList.createSubscription(EVENTS.MODEL.DATUM_CREATE + ':' + target.id , this.receiveBindEvent.bind(this));
      appSubsList.createSubscription(EVENTS.MODEL.DATUM_UPDATE + ':' + target.id , this.receiveBindEvent.bind(this));
      appSubsList.createSubscription(EVENTS.MODEL.DATUM_REMOVE + ':' + target.id , this.receiveBindEvent.bind(this));
    }
