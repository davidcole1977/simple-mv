...
model = {
  set: function set (keypath, value) {
    ...
    // TODO:
    // IF VALUE HAS NOT CHANGED, RETURN WITH NO VALUE SET OR EVENT EMITTED
    ...
    // TODO: only emit model events – just need to ensure the right data is published with the event
  ...
  remove: function remove (key) {
    if (typeof key !== 'string') {
      throw new Error('remove(key) expects a single string as an argument');
    }
  ...
  // TODO: remove this method – we only really need model event
  emitDatumEvent: function emitDatumEvent(keypath, eventType) {
    ...
    this.subsList.publish(keypath, publishParams);
