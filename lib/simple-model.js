...
modelPrototype = {
  ...
  extend: function extend (options) {
    // return this.create.bind(null, options); ????
    return null;
  },

  create: function create () {
    // proxy the regular create() function, adjusted to be bound to this????
    return null;
  }
...
function create (options) {
  var newModel,
      attemptedPrototypeOverride = false;
  ...
  // strip protected options (model prototype properties) from options
  // REFACTOR ME INTO NICE, NEAT, SEPARATE FUNCTION attemptedPrototypeOverride() OR SIMILAR
  Object.keys(modelPrototype).forEach(function (prototypeKey) {
    if (typeof options[prototypeKey] !== 'undefined') {
      attemptedPrototypeOverride = true;
    }
  });

  if (attemptedPrototypeOverride) {
...
function extend (options) {
  // return create.bind(null, options); ????
  return null;
}
