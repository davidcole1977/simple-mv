...
function removeDeep (data, keys, index) {
  if (index < keys.length - 1) {
    removeDeep(data[keys[index]], keys, index + 1);

  // if it's an array entry, entirely remove that element
  } else if (sh.stringContainsOnlyDigits(keys[index])) {
    data.splice(parseInt(keys[index], 10), 1);

  // else it's an object attribute, so just delete it
  } else {
...
prototype = {
  ...
  remove: function remove (keypath) {
    ...
    // if datum doesn't exist, throw error
    try {
      this.get(keypath);
    } catch (error) {
      throw error;
