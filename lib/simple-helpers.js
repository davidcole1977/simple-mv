...
module.exports = {
  ...
  isValidKeypath: function (keypath) {
    return null;
  },

  /**
   * if keypath2 is a descendent of keypath1, returns just the difference / descendent portion
   */
  getKeypathDescendentPortion: function getKeypathDescendentPortion (keypath1, keypath2) {
    var intersectionRegex;

    if (keypath1 == '*') {
      return '';
    }

    intersectionRegex = new RegExp('^' + keypath1);

    return keypath2.replace(intersectionRegex, '');
  },

  /**
   * Returns true if keypath2 is the same as keypath1, or a descendent of keypath1
   */
  keypathIsSameOrDescendent: function keypathIsSameOrDescendent (keypath1, keypath2) {
    var comparisonRegex;

    // '*' is wildcard meaning the whole model / all keypaths in model
    if (keypath1 == '*') {
      return true;
    }

    comparisonRegex = new RegExp('^' + keypath1);
    return comparisonRegex.test(keypath2);
  }
};
